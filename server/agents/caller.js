...
export function hasVapi() {
  return Boolean(process.env.VAPI_API_KEY && process.env.VAPI_PHONE_NUMBER_ID && process.env.VAPI_ASSISTANT_ID && process.env.RECEPTIONIST_PHONE)
}
...
function setCall(patch) {
  updateState((s) => { s.call = { ...s.call, ...patch } })
  pushEvent('call_status', patch)
}

function pushChunk(speaker, text) {
  const chunk = { speaker, text }
...
export async function startCall({ branch = 'B2' } = {}) {
  updateState((s) => { s.transcript = []; s.metrics.callsMade += 1 })
...
const SCRIPTS = {
  B1: [
    ['assistant', "Bonjour, ici l'assistante vocale d'AccessTrip pour Mme Camille Moreau. Cet appel est enregistré. Je confirme la chambre 104 accessible pour le 12 septembre ?"],
  ...
  B2: [
    ['assistant', "Bonjour, ici l'assistante vocale d'AccessTrip pour Mme Camille Moreau. Cet appel est enregistré. Je confirme la chambre 104 accessible pour le 12 septembre ?"],
    ['human', 'Ah… alors justement, la 104 a été réattribuée. Nous n\'avons plus de chambre accessible ce soir-là.'],
    ['assistant', 'Je comprends. Existe-t-il une alternative accessible avec douche à l\'italienne ?'],
    ['human', 'Non, désolée, rien de disponible chez nous ce soir-là.'],
    ['assistant', 'Merci de votre franchise. Un conseiller va rappeler pour organiser une solution. Bonne journée.'],
  ],
  B3: [
    ['assistant', "Bonjour, ici l'assistante vocale d'AccessTrip pour Mme Camille Moreau. Cet appel est enregistré. Je confirme la chambre 104 accessible pour le 12 septembre ?"],
    ['human', 'Euh… il faudrait voir avec ma collègue demain, je ne peux pas vous dire là.'],
    ['assistant', 'Très bien. Pouvez-vous confirmer si la chambre est accessible avec douche à l\'italienne ?'],
