...
async function extract(transcript) {
  ...
  try {
    const data = await claudeJSON({ system: SYSTEM, user: transcriptText(transcript), schema: EXTRACT_SCHEMA })
    return { data, source: 'claude' }
  } catch (err) {
    return { data: fallbackExtract(transcript), source: 'fallback', error: err.message }
  }
}
...
export async function runExtractionAndRecover(transcript) {
