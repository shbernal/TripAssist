 ...
 * GIF is a 256-colour, per-frame-palette format, so it can only be pushed so
 * far; the sharpness comes from a max-quality gifski encode with only a light
 * lossy touch (the old lossy:78 was mushing the dark UI). lossy is gifski's
 * "quality" of its LZW loss: HIGHER = less loss, so 96 is a gentle squeeze.
 */
