...
export default function DemoPanel({ state, reload }) {
  ...
  async function postBody(url, body, label) {
    setBusy(true)
    setStatus(`${label}…`)
    try {
    ...
    <div className="page">
      ...
      <section className="panel" aria-labelledby="guided-title" style={{ marginBottom: '1rem' }}>
        ...
        <p className="muted" style={{ marginTop: '-0.3rem' }}>
          Déroulé automatique et narré des trois flux (perturbation, appel IA, vision). Idéal pour la scène.
        </p>
        <button
          type="button"
          className="primary"
      ...
      <section className="panel" aria-labelledby="demo-controls-title">
        ...
        <div className="demo-actions">
          ...
          <button
            type="button"
            onClick={() => postBody('/api/call/start', { branch: 'B2' }, 'Appel IA (chambre indisponible)')}
            disabled={busy}
          >
        ...
        <p className="demo-note">
          État actuel : {state.trip.steps.length} étapes ·
          {' '}{state.ledger.length} confirmation(s) au registre ·
          {' '}{state.agentLog.length} ligne(s) de journal.
        </p>
      </section>
      ...
      <section className="panel" aria-labelledby="scenarios-title" style={{ marginTop: '1rem' }}>
        ...
        <p className="muted" style={{ marginTop: '-0.3rem' }}>
          Injectez un incident — le watchdog détecte, le planificateur remédie, chaque type a sa cascade.
        </p>
        <div className="demo-actions">
          {SCENARIOS.map((sc) => (
            <button
...
const STATUSES = ['identified', 'contacted', 'confirmed', 'reconfirmed', 'in_progress', 'done', 'at_risk', 'failed']

// M6 "invisible insurance": force any step to any status manually if a live
// event fails on stage.
function ForceControls({ steps, postBody, busy }) {
  const [stepId, setStepId] = useState(steps[0]?.id || 's1')
  const [status, setStatus] = useState('at_risk')
  const [reason, setReason] = useState('')

  return (
    <section className="panel" aria-labelledby="force-title" style={{ marginTop: '1rem', borderStyle: 'dashed' }}>
      ...
      <div className="demo-actions" style={{ alignItems: 'flex-end' }}>
        <label>Étape<br />
          <select value={stepId} onChange={(e) => setStepId(e.target.value)} disabled={busy}>
            {steps.map((s) => <option key={s.id} value={s.id}>{s.id} — {s.title.slice(0, 28)}</option>)}
          </select>
        </label>
        <label>Statut<br />
        ...
        <label>Motif (optionnel)<br />
          <input type="text" value={reason} onChange={(e) => setReason(e.target.value)} disabled={busy} placeholder="ex. TGV retardé" />
        </label>
        <button
          type="button"
          onClick={() => postBody('/api/demo/force-step', { stepId, status, reason: reason || null }, `Forçage ${stepId} → ${status}`)}
          disabled={busy}
        >
          Forcer
        </button>
