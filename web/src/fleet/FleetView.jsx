...
export default function FleetView({ state }) {
  ...
  const camille = {
    id: 'camille', name: state.traveler.name, category: 'Moteur',
    profileShort: 'Fauteuil électrique · douche italienne', client: 'AXA Assistance', tripType: 'Loisir',
    route: 'Paris → Nice', dates: '12–15 septembre',
    steps: state.trip.steps.map((s) => s.status), confirmations: state.ledger.length, live: true,
  }
  const all = [camille, ...(state.fleet || [])]
  const clients = ['all', ...Array.from(new Set(all.map((t) => t.client)))]

  const travelers = useMemo(() => all.filter((t) => {
    if (statusFilter !== 'all' && overall(t.steps).cls !== statusFilter) return false
    if (clientFilter !== 'all' && t.client !== clientFilter) return false
    if (q) {
      const hay = `${t.name} ${t.route} ${t.profileShort} ${t.client} ${t.tripType}`.toLowerCase()
  ...
  return (
    <div className="page">
      <div className="fleet-head">
        <div>
          ...
          <p className="muted" style={{ margin: '0.2rem 0 0' }}>
            ...
            <span className="scale-note"> Échantillon de {all.length} voyageurs ; la plateforme orchestre des milliers de trajets en parallèle.</span>
          </p>
        </div>
      </div>
      ...
      <div className="fleet-filters" role="search">
        <input
          type="text" className="fleet-search" placeholder="Rechercher un voyageur, une ligne, un profil…"
          value={q} onChange={(e) => setQ(e.target.value)} aria-label="Rechercher un voyageur"
        />
        ...
        <select value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} aria-label="Filtrer par client">
          {clients.map((c) => <option key={c} value={c}>{c === 'all' ? 'Tous les clients' : c}</option>)}
        </select>
        <span className="fleet-count muted">{travelers.length} / {all.length}</span>
      </div>
      ...
      <div className="fleet-grid">
        ...
        {travelers.length === 0 && <p className="muted">Aucun voyageur ne correspond à ces critères.</p>}
      </div>
    </div>
  )
}
...
function FleetCard({ traveler, onOpen }) {
  ...
  return (
    <button type="button" className="fleet-card" onClick={onOpen} aria-label={`Ouvrir le voyage de ${traveler.name}`}>
      <div className="fleet-card-head">
        ...
        <span className="badge" data-status={o.cls === 'in_progress' ? 'in_progress' : o.cls}>{o.label}</span>
      </div>
